...
public class Spider : MonoBehaviour {
    ...
    private Rigidbody rb;   // get rigid body

    [Header("Debug")]
    public bool showDebug;  // show debug or not

    [Header("Movement")]
    [Range(1, 10)]
    public float walkSpeed; //spider walk speed
    [Range(1, 10)]
    public float runSpeed;  //spider run speed
    [Range(1, 5)]
    public float turnSpeed; // spider turn speed
    [Range(0.001f, 1)]
    public float walkDrag;  // spider moving drag

    [Header("Grounding")]
    public CapsuleCollider capsuleCollider; // the collision capsule
    [Range(1, 10)]
    public float groundNormalAdjustSpeed;   // ground adjust speed
    [Range(1, 10)]
    public float forwardNormalAdjustSpeed;  // forward adjust speed
    public LayerMask walkableLayer;         //what layer can it walk on
    [Range(0, 1)]
    public float gravityOffDistance;        //gravity off distance

    [Header("IK Legs")]
    public Transform body;  //get transform of the spider body
    public IKChain[] legs;  //get IKChain of spider legs

    [Header("Body Offset Height")]
    public float bodyOffsetHeight;  //the height of the spider

    [Header("Leg Centroid")]
    public bool legCentroidAdjustment;  //centre adjustmeny of the legs
    [Range(0, 100)]
    public float legCentroidSpeed;      //centre leg speed
    [Range(0, 1)]
    public float legCentroidNormalWeight;   //centre normal weight
    [Range(0, 1)]
    public float legCentroidTangentWeight;  // centre tangent weight

    [Header("Leg Normal")]
    public bool legNormalAdjustment;    //normal leg adjustment
    [Range(0, 100)]
    public float legNormalSpeed;        //normal leg speed
    [Range(0, 1)]
    public float legNormalWeight;       //normal leg weight

    private Vector3 bodyY;  //y vector of the spider
    private Vector3 bodyZ;  //z vector of the spider

    [Header("Breathing")]
    public bool breathing;  //breathing cooldown of the spider
    [Range(0.01f, 20)]
    public float breathePeriod; //breathing period of the spider
    [Range(0, 1)]
    public float breatheMagnitude;  //breathing magnitude of the spider

    [Header("Ray Adjustments")]
    [Range(0.0f, 1.0f)]
    public float forwardRayLength;  //forward rat length
    [Range(0.0f, 1.0f)]
    public float downRayLength;     //down ray length
    [Range(0.1f, 1.0f)]
    public float forwardRaySize = 0.66f;    //forward ray size
    [Range(0.1f, 1.0f)]
    public float downRaySize = 0.9f;        //down ray size
    private float downRayRadius;            //down ray radius
    ...
    private void move(Vector3 direction, float speed) {
        ...
        if (direction != Vector3.zero) {
            ...
            // Debug.Log("directionDamp 1: " + directionDamp); Debug.Log("distance1: " + distance);

            distance = Mathf.Clamp(distance, 0, 0.99f * downRayRadius);
            direction = distance * (direction / magnitude);

            // Debug.Log("directionDamp 2: " + directionDamp); Debug.Log("distance2: " + distance);
